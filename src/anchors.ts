...
export function sideAnchor(node: IUI, side: ConnectorSide): IPointData {
  const b = node.worldBoxBounds;
  switch (side) {
    case "top":
...
export function nearestSideMidAnchor(node: IUI, toward: IPointData): IPointData {
  // 选择“最近的一条边”，并返回该边的中点（不是射线交点）
  // 规则：比较 4 个边中点到 toward 的距离，取最小值（比 dx/dy 比例更稳定）
  return sideAnchor(node, chooseNearestSide(node, toward));
}

export function chooseNearestSide(node: IUI, toward: IPointData): ConnectorSide {
...
export function nearestEdgeAnchor(node: IUI, toward: IPointData): IPointData {
  ...
  const crRaw = (node as any).cornerRadius as undefined | number | number[];
  if (!crRaw) return p;

  const to4 = (r: number | number[]) => {
    if (Array.isArray(r)) {
      const tl = r[0] ?? 0;
      const tr = r[1] ?? tl;
      const br = r[2] ?? tl;
  ...
  if (!inTopLeft && !inTopRight && !inBottomRight && !inBottomLeft) return p;
...
export function resolveAnchor(
  node: IUI,
  spec: ConnectorAnchorSpec | ((node: IUI, toward: IPointData) => IPointData),
  toward: IPointData
): IPointData {
  if (typeof spec === "function") return spec(node, toward);
  if (spec === "center" || spec.type === "center") return centerAnchor(node);
