...
export function inferSideByPoint(node: IUI, p: IPointData): ConnectorSide {
...
export function getCubicBezierControls(
  from: IPointData,
  to: IPointData,
  fromSide: ConnectorSide,
  toSide: ConnectorSide,
  curvature: number
): { c1: IPointData; c2: IPointData } {
  /**
  ...
  const tension = clamp(Math.abs(curvature) / 0.4, 0.5, 3);
  ...
  if (isOpposite) {
    ...
    if (o1.x !== 0) {
      ...
      const k = Math.min(clamp(base, 8, 360), primary * 0.9);
...
export function getSideFromAnchorSpec(
  spec: ConnectorAnchorSpec | ((node: IUI, toward: IPointData) => IPointData)
): ConnectorSide | undefined {
  if (!spec || typeof spec === "function") return undefined;
  if (typeof spec === "string") return undefined;
  if (spec.type === "side") return spec.side;
  return undefined;
}
