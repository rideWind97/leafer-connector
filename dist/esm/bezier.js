...
export function inferSideByPoint(node, p) {
...
export function buildCubicBezierPath(from, to, fromSide, toSide, curvature) {
    const { c1, c2 } = getCubicBezierControls(from, to, fromSide, toSide, curvature);
    return `M ${from.x} ${from.y} C ${c1.x} ${c1.y} ${c2.x} ${c2.y} ${to.x} ${to.y}`;
}
export function getCubicBezierControls(from, to, fromSide, toSide, curvature) {
    /**
...
export function getSideFromAnchorSpec(spec) {
