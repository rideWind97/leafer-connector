...
exports.buildRoundedPolylinePath = buildRoundedPolylinePath;
exports.buildOrthogonalBetween = buildOrthogonalBetween;
function expandRect(r, pad) {
    return { x: r.x - pad, y: r.y - pad, width: r.width + pad * 2, height: r.height + pad * 2 };
}
function rectMinMax(r) {
    return { x1: r.x, y1: r.y, x2: r.x + r.width, y2: r.y + r.height };
}
function axisSegmentIntersectsRect(a, b, r) {
...
function polylineSegmentLengths(points) {
    const lens = [];
    for (let i = 0; i < points.length - 1; i++) {
        const a = points[i];
        const b = points[i + 1];
...
function buildOrthogonalBetween(from, to, avoidRects, options) {
    /**
     * 智能正交路由（简化版）
     *
     * 思路：
    ...
    const score = (pts) => {
        ...
        for (let i = 0; i < pts.length - 1; i++) {
            ...
            for (const r of avoidRects) {
                if (axisSegmentIntersectsRect(a, b, r))
                    s += intersectionPenalty;
            }
        }
        // 长直线惩罚
