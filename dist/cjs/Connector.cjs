...
class Connector extends leafer_editor_1.Group {
    constructor(app, options) {
        ...
        if (options.fromPorts?.length)
            (0, ports_1.setNodePorts)(this.fromNode, options.fromPorts);
        if (options.toPorts?.length)
            (0, ports_1.setNodePorts)(this.toNode, options.toPorts);
        ...
        this.fromHandle = new leafer_editor_1.Rect({ ...handleStyle });
        this.toHandle = new leafer_editor_1.Rect({ ...handleStyle });
    ...
    buildCandidatePoints(node, opt, nodeRectLocal, otherRectLocal) {
        ...
        // portId
        if (opt?.portId) {
            const ports = (0, ports_1.getNodePorts)(node);
            const found = ports?.find(p => p.id === opt.portId);
            if (found) {
                const wp = (0, ports_1.portAnchor)(node, found);
                const lp = this.getLocalPoint(wp);
                const side = (0, bezier_1.inferSideByPoint)(node, wp);
                const dir = sideOutDir(side);
                const paddingPointLocal = { x: lp.x + dir.x * padding, y: lp.y + dir.y * padding };
                return [
                    {
    ...
    positionLabel(pointsLocal) {
        if (!this._label)
            return;
        const mid = (0, route_1.polylineMidpoint)(pointsLocal);
        this._labelMid = mid;
    ...
    bindInteractions() {
        ...
        const onHandleDrag = (which) => {
            ...
            handle.on_(leafer_editor_1.DragEvent.DRAG, () => {
                const hx = handle.x ?? 0;
                const hy = handle.y ?? 0;
            ...
            handle.on_(leafer_editor_1.DragEvent.END, () => {
                const hx = handle.x ?? 0;
                const hy = handle.y ?? 0;
                const hw = handle.width ?? this._handleSize;
                const hh = handle.height ?? this._handleSize;
                ...
                const exclude = new leafer_editor_1.LeafList([this, this.wire, this.fromHandle, this.toHandle, this._label]);
